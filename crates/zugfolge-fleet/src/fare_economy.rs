...
pub struct FareInspectionEconomyV1 {
    ...
    pub proof_handling_cost_cents: String,
...
impl FareInspectionEconomyV1 {
    pub fn validate(&self) -> bool {
        self.schema_version == "fare-inspection-economy/v1"
            && self.minimum_claim_cents == "6000"
            && self.ordinary_fare_multiplier == 2
            && self.reduced_claim_cents == "700"
            && self.proof_window_days == 7
            && self.day_length_ms == 86_400_000
            && [
                &self.handling_cost_cents,
                &self.proof_handling_cost_cents,
                &self.police_handling_cost_cents,
            ]
            .iter()
            .all(|s| cents(s))
            && self.full_payment_basis_points <= 10_000
            && self.partial_payment_basis_points <= 10_000
            ...
            && self.write_off_delay_ms > self.payment_delay_ms
            && self.valid_proof_submission_basis_points <= 10_000
            && self.valid_proof_delay_ms > 0
            && self.premium_multiplier_basis_points <= 40_000
            && self.positive_daily_cap_basis_points <= 50
            && self.revenue_allocation == "uniform_settled_service_interval/v1"
    }
}

pub fn deserialize_optional_fare_inspection<'de, D>(
    deserializer: D,
) -> Result<Option<FareInspectionEconomyV1>, D::Error>
where
