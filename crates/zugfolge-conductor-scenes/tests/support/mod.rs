//! Explicit fictional rail test fixture, using the real M14.1 OperationalWorld.
#![allow(dead_code)]
use std::collections::{BTreeMap, BTreeSet};
use zugfolge_sim::operational::*;

pub fn set(values: &[&str]) -> BTreeSet<String> {
    values.iter().map(|value| (*value).to_owned()).collect()
}

pub fn release() -> OperationalInfraRelease {
