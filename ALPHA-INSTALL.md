...
kanonischen Wrappermodus `--keycloak-after-schema36`; das darin gekapselte
...
danach im selben digestgebundenen Prozess die Migrationen 0032 bis 0036 bis
zum exakten Zielkopf 36 anwenden:
...
bash tools/alpha-ops/compose-with-map-release-env.sh \
  --schema36-after-cold -f /opt/zugfolge/compose.yml
bash tools/alpha-ops/compose-with-map-release-env.sh \
  --keycloak-after-schema36 -f /opt/zugfolge/compose.yml
```
...
sein. Der Modus prüft zuerst erneut exakt Schema 36, erstellt dann das
...
über `--keycloak-recover-after-schema36` abgeschlossen werden.
...
die Writer gestoppt. `--prepare-v2-hot` prüft Schema 36 und denselben gültigen
...
Satz weltgebundener Schema-36-Tabellen einschließlich des öffentlichen
...
unveränderten Schema-36-Hot-Restore verwendet und **keine** Keycloak-
...
Schema 36 einschließlich des persistenten Fahrzeugregisters aus Migration 0036. Die genauen Vorbedingungen stehen unter
