...
function worldContracts(state: JourneyViewState): string {
  ...
  const cards = contracts.map((contract) => {
    ...
    const joined = selected && state.hasActiveOperator === true;
  ...
  const contractCount = `${contracts.length} ${contracts.length === 1 ? "Weltvertrag" : "Weltverträge"}`;
  return `<section class="world-contracts journey-card" aria-labelledby="world-contract-title"><div class="journey-heading"><div><p class="eyebrow">${eyebrow}</p><h2 id="world-contract-title">${title}</h2></div><span class="state-word">${contractCount}</span></div><p>${introduction}</p><div class="world-contract-grid">${cards}</div></section>`;
}
