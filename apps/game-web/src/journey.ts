...
import { renderCooperationSurface, vehiclePassportFragment, type CooperationSurfaceState } from "./cooperation.js";
...
function companyExitForm(state: JourneyViewState): string {
  const vehicles = state.cooperation?.ownedVehicles;
  if (vehicles === undefined || !state.activeOperatorId) return "";
  return `<details class="market-compose" data-preserve-disclosure="company-exit"><summary>Betrieb aufgeben</summary>
    <p>Beende dein Unternehmen dauerhaft, sobald seine offenen Verpflichtungen erfüllt und alle Fahrzeuge frei von Betriebsbindungen sind. Mietfahrzeuge gehen zurück. Deine eigenen Fahrzeuge werden mit ihrer Geschichte weiterverkauft.</p>
    <form id="company-exit-form" class="m12-form" data-preserve-draft>
      ${vehicles.map((vehicle) => `<label class="m12-field"><span>Verkaufspreis · ${escapeHtml(vehicle.classDesignation)} · ${escapeHtml(vehicle.vehicleId)} · Euro</span><input name="price:${escapeHtml(vehicle.vehicleId)}" inputmode="decimal" required placeholder="Dein Angebotspreis" value="${vehicle.valueCents === null ? "" : `${BigInt(vehicle.valueCents) / 100n},${String(BigInt(vehicle.valueCents) % 100n).padStart(2, "0")}`}"></label>`).join("")}
      <label><input type="checkbox" name="confirmExit" value="yes" required> Ich möchte dieses Unternehmen dauerhaft beenden.</label>
      <button class="secondary" type="submit"${state.busy ? " disabled" : ""}>Betriebsaufgabe prüfen …</button>
    </form><p>Die Preise sind Angebote, kein sofortiger Erlös. Fahrzeugidentitäten und Lebensläufe bleiben öffentlich erhalten.</p></details>`;
}
