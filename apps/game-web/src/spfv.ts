...
import { routeImportMarkup, type ImportedRoute } from "./route-import.js";
...
export interface SpfvCatalog {
  ...
  readonly stops: readonly { readonly id: string; readonly label: string; readonly code?: string }[];
...
export interface SpfvPreview {
  ...
  readonly replacementTrips: readonly { readonly trainId: string; readonly trainNumber: number; readonly departureS: number; readonly originLabel: string | null; readonly destinationLabel: string | null }[];
...
export function parseSpfvCatalog(value: unknown, worldId: string, operatorId: string): SpfvCatalog {
  ...
  for (const entry of data["stops"]) { const row = object(entry); if (!identifier(row["id"]) || !identifier(row["label"]) || (row["code"] !== undefined && typeof row["code"] !== "string")) throw new Error("Ein freigegebener Halt ist unvollständig."); }
  ...
  for (const entry of data["lines"]) {
    ...
    if ((row["departureFlexibilityS"] !== undefined && (!count(row["departureFlexibilityS"]) || Number(row["departureFlexibilityS"]) > 7200))
      || (row["extraRunningTimeS"] !== undefined && (!count(row["extraRunningTimeS"]) || Number(row["extraRunningTimeS"]) > 3600))) throw new Error("Der gespeicherte Planungsspielraum ist ungültig.");
    if (row["viaStationIds"] !== undefined && (!Array.isArray(row["viaStationIds"]) || row["viaStationIds"].length > 510 || !row["viaStationIds"].every(identifier)
      || new Set(row["viaStationIds"]).size !== row["viaStationIds"].length)) throw new Error("Der gespeicherte Fahrweg ist unvollständig.");
...
export function parseSpfvPreview(value: unknown, worldId: string, operatorId: string): SpfvPreview {
  ...
  const capacity = object(data["capacityFacts"]);
  const replacements = data["replacementTrainIds"];
  const trips = data["replacementTrips"];
  if (!["standardSeats", "premiumSeats", "bicycleSpaces", "wheelchairSpaces"].every((key) => count(capacity[key]))
    || !count(Number(capacity["standardSeats"]) + Number(capacity["premiumSeats"]))
    || (data["capacity"] !== null && Number(capacity["standardSeats"]) + Number(capacity["premiumSeats"]) !== data["capacity"])
    || !Array.isArray(replacements) || replacements.length > 256 || !replacements.every((id) => identifier(id) && id.length <= 300)
    || new Set(replacements).size !== replacements.length
    || !Array.isArray(trips) || trips.length !== replacements.length || !trips.every((item, index) => {
      const trip = object(item);
      return trip["trainId"] === replacements[index] && count(trip["trainNumber"]) && Number(trip["trainNumber"]) > 0
        && count(trip["departureS"]) && ["originLabel", "destinationLabel"].every((key) => trip[key] === null || identifier(trip[key]));
    })) throw new Error("Geprüfte Kapazität oder betroffene Fahrten sind unvollständig. Bitte erneut prüfen.");
  return data as unknown as SpfvPreview;
}
export function parseSpfvDraft(fields: Readonly<Record<string, string>>, stopIds: readonly string[], catalog: SpfvCatalog, referenceTrainId?: string, viaStationIds?: readonly string[]): SpfvLineDraft {
  ...
  if (viaStationIds !== undefined) {
    if (viaStationIds.length > 510 || !viaStationIds.every(identifier) || new Set(viaStationIds).size !== viaStationIds.length
      || viaStationIds.includes(stopIds[0]!) || viaStationIds.includes(stopIds.at(-1)!)) throw new Error("Der vorgegebene Fahrweg enthält ungültige oder wiederholte Betriebsstellen.");
    const route = [stopIds[0]!, ...viaStationIds, stopIds.at(-1)!];
    let previous = -1;
    for (const stop of stopIds) {
      const index = route.indexOf(stop);
      if (index <= previous) throw new Error("Alle Halte müssen in Fahrtrichtung auf dem übernommenen Fahrweg liegen. Entferne den Fahrweg, um eine andere Route zu planen.");
      previous = index;
    }
  }
  ...
  const flexibility = fields["departureDelayMinutes"] === undefined && fields["extraRunningMinutes"] === undefined ? {} : parsePlanningFlexibility(fields, { departureMinutes: 0, runningMinutes: 0 });
  return { name, stopIds: [...stopIds], headwayS, fareCents, formationId, validFromS, validUntilS, ...flexibility, ...(viaStationIds === undefined ? {} : { viaStationIds: [...viaStationIds] }), ...(fields["lineId"] ? { lineId: fields["lineId"] } : {}), ...(referenceTrainId ? { referenceTrainId } : {}) };
}
export function spfvStopsForRoute(route: ImportedRoute, stopIds: readonly string[], catalog: SpfvCatalog): string[] {
  if (route.worldId !== catalog.worldId || route.releaseId !== catalog.releaseId) throw new Error("Der Laufweg gehört zu einem anderen Stand deiner Spielwelt. Lade die Planung neu.");
  const origin = route.stations[0]!.id; const destination = route.stations.at(-1)!.id;
  if (![origin, destination].every((id) => catalog.stops.some((stop) => stop.id === id))) throw new Error("Start und Ziel müssen als Fernverkehrshalte in deiner Spielwelt freigegeben sein.");
  const selected = new Set([origin, ...stopIds, destination]);
  const result = route.stations.filter((station) => selected.has(station.id) && catalog.stops.some((stop) => stop.id === station.id)).map((station) => station.id);
  if (result.length > 32) throw new Error("Der Fahrweg würde mehr als 32 ausgewählte Halte enthalten. Entferne zuerst einen Halt.");
  return result;
}
...
export interface SpfvViewState {
  ...
  readonly selectedTrainId?: string; readonly submission?: SpfvSubmission;
  readonly route?: ImportedRoute;
}
const numberLabel = (value: number | null): string => value === null ? "nicht verfügbar" : value.toLocaleString("de-DE");
export function spfvPreviewMarkup(preview: SpfvPreview, draft: SpfvLineDraft, catalog: SpfvCatalog, route?: ImportedRoute): string {
  ...
  const path = draft.viaStationIds === undefined ? "" : `<div><dt>Vorgegebener Fahrweg</dt><dd><ol class="route-import-points" tabindex="0" aria-label="Geprüfter Fahrweg">${[draft.stopIds[0]!, ...draft.viaStationIds, draft.stopIds.at(-1)!].map((id) => `<li>${escapeHtml(route?.stations.find((station) => station.id === id)?.name ?? catalog.stops.find((stop) => stop.id === id)?.label ?? id)} · ${draft.stopIds.includes(id) ? "Halt" : "Durchfahrt"}</li>`).join("")}</ol></dd></div>`;
  const affected = preview.replacementTrips.length === 0 ? "Keine bestehenden Fahrten werden ersetzt."
    : `<ul class="spfv-affected-trains" tabindex="0" aria-label="Zu ersetzende Fahrten">${preview.replacementTrips.map((trip) => `<li><strong>Zug ${trip.trainNumber}</strong> · ${spfvTime(trip.departureS)}<br>${escapeHtml(trip.originLabel ?? "Start nicht verfügbar")} → ${escapeHtml(trip.destinationLabel ?? "Ziel nicht verfügbar")}</li>`).join("")}</ul>`;
  return `<section class="spfv-preview zf-surface" aria-labelledby="preview-title" tabindex="-1"><p class="eyebrow">DEIN ANGEBOT IM VERGLEICH</p><h2 id="preview-title">${escapeHtml(draft.name)}</h2><p class="spfv-provenance">${source} · Stand ${spfvTime(preview.asOfS)} · ${escapeHtml(catalog.periodId)}</p><dl class="spfv-facts"><div><dt>Betroffene Linie</dt><dd>${escapeHtml(draft.name)}</dd></div><div><dt>Zu ersetzende Fahrten</dt><dd>${affected}</dd></div>${draft.referenceTrainId === undefined ? "" : `<div><dt>Referenzfahrt</dt><dd>${escapeHtml(draft.referenceTrainId)}</dd></div>`}<div><dt>Streckenabschnitt</dt><dd>${draft.stopIds.map((id) => escapeHtml(catalog.stops.find((stop) => stop.id === id)?.label ?? id)).join(" → ")}</dd></div>${path}<div><dt>Spielraum bei Konflikten</dt><dd>Abfahrt bis zu ${(draft.departureFlexibilityS ?? 0) / 60} Minuten später · bis zu ${(draft.extraRunningTimeS ?? 0) / 60} Minuten zusätzliche Fahrzeit. Betriebshalte und längere Aufenthalte sind innerhalb dieses Rahmens möglich.</dd></div><div><dt>Zeitraum & Takt</dt><dd>${spfvTime(draft.validFromS)} bis ${spfvTime(draft.validUntilS)} · alle ${draft.headwayS / 60} min</dd></div><div><dt>Plätze je Fahrt</dt><dd>${numberLabel(preview.capacity)}</dd></div><div><dt>Komfort & Platzarten</dt><dd>${escapeHtml(formation?.label ?? "nicht verfügbar")}<br>1. Klasse: ${numberLabel(preview.capacityFacts.premiumSeats)} · 2. Klasse: ${numberLabel(preview.capacityFacts.standardSeats)} · Fahrradplätze: ${numberLabel(preview.capacityFacts.bicycleSpaces)} · Rollstuhlplätze: ${numberLabel(preview.capacityFacts.wheelchairSpaces)}</dd></div><div><dt>Fahrpreis je Abschnitt</dt><dd>${formatCents(draft.fareCents)}</dd></div></dl><div class="spfv-metrics"><div><strong>${numberLabel(preview.requestedPassengers)}</strong><span>Nachfrage im Zeitraum</span></div><div><strong>${numberLabel(preview.servedPassengers)}</strong><span>Bediente Reisende</span></div><div><strong>${numberLabel(preview.unservedPassengers)}</strong><span>Weiterhin offen</span></div></div><dl class="spfv-facts"><div><dt>Fahrgelderlös im Zeitraum</dt><dd>${preview.fareRevenueCents === null ? "nicht verfügbar" : formatCents(preview.fareRevenueCents)}</dd></div><div><dt>Betriebskosten im Zeitraum</dt><dd>${preview.costsCents === null ? "nicht verfügbar" : formatCents(preview.costsCents)}</dd></div></dl><h3>Anschlusswirkung</h3><ul>${preview.connectionEffects.map((effect) => `<li>${escapeHtml(effect)}</li>`).join("") || "<li>Keine Anschlusswirkung im Prüfstand ausgewiesen; Anschlussqualität nicht nachgewiesen.</li>"}</ul><h3>Trassenprüfung</h3><ul>${preview.conflicts.map((conflict) => `<li>${escapeHtml(conflict)}</li>`).join("") || "<li>Keine Konflikte im vorliegenden Prüfstand ausgewiesen. Die endgültige Vergabe erfolgt durch die Trassenplanung.</li>"}</ul><details><summary>Datengrundlage</summary><p>Release ${escapeHtml(preview.releaseId)}. Prognosen sind keine garantierten Erlöse. Fehlende Zahlen sind keine Nullwerte.</p></details><p>Mit der Bestätigung reichst du diese Linie zur Trassenplanung ein. Sie fährt erst nach bestätigter Trassenvergabe und betrieblicher Freigabe.</p><button id="spfv-confirm" class="zf-button primary" type="button"${preview.confirmationAllowed ? "" : " disabled"}>Linie verbindlich zur Planung einreichen</button>${preview.confirmationAllowed ? "" : "<p>Einreichung gesperrt. Behebe die genannten Voraussetzungen und prüfe dein Angebot erneut.</p>"}</section>`;
}
