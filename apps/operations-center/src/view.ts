...
function renderDecision(entry: OperationsDecision, major: boolean, selected: boolean, consumerUnavailable: boolean): string {
  ...
  return `<article id="event-${entry.sequence}" class="decision-card${major ? " decision-card--major" : ""}${selected ? " is-selected" : ""}" tabindex="0" data-decision-id="${escapeHtml(entry.decisionId)}">
    ...
    <button class="quiet-button" data-open-override="${escapeHtml(entry.decisionId)}"${consumerUnavailable ? " disabled" : ""}>Einzelfall entscheiden</button>
  </article>`;
}
...
function reportMarkup(report: DailyReportRow): string {
  ...
  return `<article class="report-card"><header><h4>${escapeHtml(report.serviceDay)}</h4>${badge(`${value.trainRuns.total} Fahrten`, "neutral", "train")}</header>
    ...
    ${value.evidenceComplete === false ? `<p role="status">Der Leistungsnachweis ist unvollständig. Dieser Bericht ist noch nicht zur Vertragsabrechnung geeignet.</p>` : ""}<p>Ertrag ${escapeHtml(value.settlements.revenueCents)} ct · Kosten ${escapeHtml(value.settlements.costCents)} ct · Vertragswirkung ${escapeHtml(value.settlements.contractPenaltyCents)} ct</p>
...
export function renderApp(state: ViewState): string {
  ...
  return `<div class="shell">
    ...
    <main id="main">
      <header class="topbar"><div><p class="eyebrow">EVU-BETRIEBSZENTRALE</p><h1>${panelTitle}</h1></div><div class="topbar-actions"><div class="operations-context"><span><small>WELT</small><strong>${escapeHtml(state.worldId)}</strong></span><span><small>EVU</small><strong>${escapeHtml(operator?.name ?? "nicht verfügbar")}</strong></span><span><small>VERFÜGBAR</small><strong>${escapeHtml(operator === undefined ? "—" : formatAvailableFinance(operator.finance))}</strong></span></div>${consumerUnavailable ? badge("Programmausführung nicht verfügbar", "danger", "warning") : active === undefined ? badge("Kein aktives Programm", "danger", "warning") : badge(`Programm v${active.version} aktiv`, "success", "check")}<button id="refresh" class="quiet-button">Aktualisieren</button></div></header>
    ...
    <dialog id="override-dialog" aria-labelledby="override-title"><form method="dialog"><header><div><p class="eyebrow">EINZELFALL</p><h2 id="override-title">Entscheidung übersteuern</h2></div><button value="cancel" formnovalidate class="icon-button" aria-label="Dialog schließen">${icon("close")}</button></header><p>Der Override gilt nur für die gewählte Entscheidung und durchläuft dieselben Kapazitäts-, Fahrzeug-, Personal-, Vertrags- und Kostengrenzen.</p><input id="override-decision" type="hidden" value="${escapeHtml(state.selectedDecisionId)}"><label>Maßnahme<select id="override-action">${ACTIONS.map((value) => option(value, "request_reroute")).join("")}</select></label><label>Begründung<textarea id="override-reason" minlength="8" maxlength="500" required></textarea></label><footer><button value="cancel" formnovalidate class="quiet-button">Abbrechen</button><button id="submit-override" value="default" class="danger-button"${consumerUnavailable ? " disabled" : ""}>Einzelfall senden</button></footer></form></dialog>
  </div>`;
}
