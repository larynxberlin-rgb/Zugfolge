...
export interface PassengerManifest {
  ...
  readonly periodId: string; readonly asOfS: number; readonly source: DemandSource;
...
export function parsePassengerManifest(value: unknown, worldId: string, operatorId: string, trainId: string): PassengerManifest {
  ...
  if (data["schemaVersion"] !== "zugfolge-passenger-manifest-view/v1" || data["worldId"] !== worldId || data["operatorId"] !== operatorId || data["trainId"] !== trainId || !text(data["periodId"]) || !Number.isSafeInteger(data["asOfS"]) || !["forecast", "observed", "assumption"].includes(String(data["source"])) || !Array.isArray(data["items"]) || data["items"].length > 50 || !(data["nextCursor"] === null || text(data["nextCursor"]))) throw new Error("Berechtigte Fahrgastliste ist nicht verfügbar.");
  for (const item of data["items"]) {
    const row = record(item);
...
export function passengerManifestMarkup(data: PassengerManifest): string {
  ...
  return `<p>${sources[data.source]} · ${escapeHtml(data.periodId)} · Stand ${demandTime(data.asOfS)}</p><p>${data.source === "observed" ? "Fahrgastkennungen dieser Fahrt." : "Prognostizierte Fahrgastkennungen im berechneten Abschnitt."} Nur dein berechtigtes Unternehmen kann diese Liste öffnen.</p><ul class="demand-segments">${data.items.map((passenger) => `<li><strong>${escapeHtml(passenger.passengerId)}</strong><span>${escapeHtml(passenger.originLabel)} → ${escapeHtml(passenger.destinationLabel)}</span><small>${passenger.seatClass === "first" ? "1." : "2."} Klasse${passenger.spaceNeeds.length === 0 ? "" : ` · ${passenger.spaceNeeds.map((need) => escapeHtml(needs[need] ?? need)).join(", ")}`}</small></li>`).join("") || "<li>Keine Fahrgäste in diesem Manifestabschnitt.</li>"}</ul>${data.nextCursor === null ? "" : '<button type="button" data-manifest-next>Nächste 50 Fahrgäste</button>'}`;
}
