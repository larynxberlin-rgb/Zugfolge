...
export async function createConductorSessionNativeFixture(control: ConductorControlIntegration, options: {
  sceneEpochUtcTimeOfDayMs?: number;
  demandSeed?: string;
  configurationIndex?: 1 | 2 | 3;
  networkScenario?: "conductor-network-acceptance/v1";
  oneServiceContract?: { readonly lotId: string; readonly serviceDay: string; readonly requiredSeats: number; readonly terminalArrivalMs?: number };
} = {}) {
  ...
  try {
    const configurationIndex = options.configurationIndex ?? 1;
    const fixture = JSON.parse(callInteriorFixtureRust(sessionFixtureBinary, [String(configurationIndex)], {}));
    if (fixture.testOnly !== true) throw new Error("Nur explizite Testdaten sind hier erlaubt.");
    const inventory = JSON.parse(readFileSync(join(base.compiled.output, "operational-vehicle-inventory-v2.json"), "utf8"));
    const seed = fixture.materialization;
    if (options.oneServiceContract?.terminalArrivalMs !== undefined) {
      const terminal = seed.stopPlan.stops.at(-1)!;
      terminal.scheduledArrivalMs = options.oneServiceContract.terminalArrivalMs;
      terminal.scheduledDepartureMs = options.oneServiceContract.terminalArrivalMs;
    }
    if (options.networkScenario !== undefined && configurationIndex !== 1) throw new Error("Der explizite Netzkorpus bindet die drei ursprünglichen Formationen 101/103/102.");
    const networkModule = options.networkScenario === undefined ? undefined : await import(new URL("../../../tools/conductor-session/network-source.mjs", import.meta.url).href);
    const network = networkModule?.prepareConductorNetworkSource(fixture, inventory, base.compiled.authority);
    ...
    const native = sessionFixtureRuntimes();
    const route = infra.routeVersions[fixture.materialization.routeVersionId];
    const formation = inventory.formations.find((row: { vehicleIds: string[] }) => row.vehicleIds.length === 1 && row.vehicleIds[0] === `fixture-interior-vehicle-${configurationIndex}`);
    ...
    train.stopPlan = stopModule.bindPassengerStopPlan({ passenger: { trainRunId: train.id, formationVersionId: train.formationVersionId,
      formationLengthMm: 70_000, serviceOutcome: { serviceId: train.id, serviceRunId: options.oneServiceContract === undefined
        ? `${train.id}:day:0` : `${train.id}:service-day:${options.oneServiceContract.serviceDay}` } }, materialization: train,
    ...
    const extraTrains: OperationalTrainInitialization[] = [];
    if (network !== undefined) {
      const follower = { ...network.follower, operatorId: OPERATOR, stopPlan: undefined,
        dispatchInterlockingRouteId: "interlocking:train", protectionModeSelectionRuns: train.protectionModeSelectionRuns };
      follower.stopPlan = stopModule.bindPassengerStopPlan({ passenger: { trainRunId: follower.id, formationVersionId: follower.formationVersionId,
        formationLengthMm: 70_000, serviceOutcome: { serviceId: follower.id, serviceRunId: `${follower.id}:day:0` } }, materialization: follower,
        timetableRoute: { routeVersionId: "base:network-follow-fixture", legs: route.legs.slice(1), passengerStopAnchors: anchors },
        timetableStops: network.follower.stopPlan.stops.map((stop: { stationId: string; stopSequence: number; scheduledArrivalMs: number; scheduledDepartureMs: number }) => ({
          stopId: stop.stationId, stopSequence: stop.stopSequence, arrivalS: stop.scheduledArrivalMs / 1000, departureS: stop.scheduledDepartureMs / 1000 })),
        infrastructure: { routes: new Map(Object.entries(infra.routeVersions)), platforms: new Map(Object.entries(infra.platformIntervals)) },
        worldId: WORLD, infrastructureReleaseId: infra.id, timetableReleaseId: "explicit-conductor-network-fixture",
        sourcePins: { gtfsSnapshotSha256: sha(JSON.stringify(network.follower.stopPlan.stops)), timetableRoutesSha256: sha(JSON.stringify(anchors)),
          infrastructureStateHash: infraBinding.stateHash, movementRouteStateHash: infraBinding.stateHash } });
      if (follower.stopPlan === null || follower.stopPlan === undefined) throw new Error("Der echte Haltbinder konnte die fiktive Folgefahrt nicht binden.");
      extraTrains.push(follower, { ...network.crossing, operatorId: OPERATOR }, { ...network.shunting, operatorId: OPERATOR });
    }
    // Ausschließlich expliziter Abnahmevertrag: genau diese Tagesfahrt,
    // gemessene M5-Kapazität und ausdrücklich keine bestellten Anschlüsse.
    const oneServiceContract = options.oneServiceContract;
    const serviceOutcome = oneServiceContract === undefined ? undefined : {
      schemaVersion: "zugfolge-operational-service-outcome-binding/v1" as const,
      serviceId: train.id, serviceRunId: `${train.id}:service-day:${oneServiceContract.serviceDay}`,
      lotId: oneServiceContract.lotId, serviceDay: oneServiceContract.serviceDay,
      scheduledArrivalMs: train.stopPlan.stops.at(-1)!.scheduledArrivalMs,
      requiredSeats: oneServiceContract.requiredSeats, connectionAssessment: "none-contracted" as const,
    };
    if (serviceOutcome !== undefined) Object.assign(train, { serviceOutcome });
    const serviceOutcomePolicy = serviceOutcome === undefined ? undefined : {
      schemaVersion: "zugfolge-operational-service-outcome-policy/v1" as const, serviceIds: [train.id],
      vehicleCapacities: formation.vehicleIds.map((vehicleId: string) => {
        const asset = base.compiled.authority.assets.find((row) => row.id === vehicleId);
        if (asset === undefined) throw new Error("Der tatsächliche Compiler-Kapazitätsbeleg fehlt.");
        return { vehicleId, seats: asset.passenger.seats,
          sourceReference: `fleet:${base.checkpoint.stateHash}:${vehicleId}` };
      }),
    };
    const initialization: OperationalSimulationInitialization = { schemaVersion: OPERATIONAL_SIMULATION_INITIALIZE_SCHEMA,
      ...
      trains: [train as OperationalTrainInitialization, ...extraTrains], movementContinuations: [],
      ...(serviceOutcomePolicy === undefined ? {} : { serviceOutcomePolicy }) };
    ...
    // Ein eigener fiktiver Testseed gilt vor allen Deployment-/Poolpins.
    if (options.demandSeed !== undefined) input.seed = options.demandSeed;
    ...
    if (network !== undefined) {
      const follower = extraTrains[0]!;
      const stops = follower.stopPlan!.stops;
      input.services.push({ ...structuredClone(service), trainRunId: follower.id, capacity: network.followerCapacity,
        stops: stops.map((stop) => ({ stopId: stop.stopId, stationId: stop.stationId, arrivalMs: stop.scheduledArrivalMs,
          departureMs: stop.scheduledDepartureMs, passengerStop: true })) });
      input.services.push(...network.forecastServices.map((planned: Record<string, unknown>) => ({ ...planned, worldId: WORLD, operatorId: OPERATOR })));
    }
    ...
    const readModel = { async getConfig() { return { infrastructureReleaseId: infra.id }; },
      async getScheduledCall(worldId: string, stationId: string, trainId: string, timeS: number, kind: string) {
        return worldId === WORLD && input.services.some((offer: { trainRunId: string; stops: Record<string, unknown>[] }) => offer.trainRunId === trainId
          && offer.stops.some((stop) => stop["stationId"] === stationId && stop[kind === "arrival" ? "arrivalMs" : "departureMs"] === timeS * 1000)) ? { trainId } : undefined;
    ...
    return { ...base, signed, native, initialization, worker, demand, dependencies, sessions, apply, refresh, network,
