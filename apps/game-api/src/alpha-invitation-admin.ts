...
export function createAlphaInvitationAdminHandlers(options: { readonly db: IdentityDatabase; readonly keycloak: KeycloakAdminClient; readonly redirectUri: string }): Readonly<Record<"alpha_invitation_create" | "alpha_invitation_resend" | "alpha_invitation_revoke", GameAdminCommandHandler>> {
  ...
  return {
    ...
    async alpha_invitation_revoke(context) {
      ...
      await options.db.update(worldAccesses).set({ status: "revoked", revokedAt: context.now }).where(and(eq(worldAccesses.worldId, context.payload.worldId), eq(worldAccesses.keycloakSubject, input.keycloakSubject!)));
      return { state: "completed", gameAuditEventId: `${input.requestReference}:${input.keycloakSubject}` };
    },
  };
}
