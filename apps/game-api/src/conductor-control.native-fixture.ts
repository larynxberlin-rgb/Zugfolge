...
export async function createFareControlNativeFixture(options: { identityRefusalBasisPoints?: number } = {}) {
  ...
  try {
    ...
    await persistEconomyTransition(fixture.db, { ...started, expectedRevision: null, committedAt: new Date(fixture.clock.nowMs), enqueuedAt: new Date(fixture.clock.nowMs) });
    ...
    const policy = { schemaVersion: "fare-inspection-policy/v1", policyId: "explicit-test-inspection", worldId, periodId, contentHash: "",
      invalidDocumentPresentedBasisPoints: 0, identityRefusalBasisPoints: options.identityRefusalBasisPoints ?? 0, concreteDangerBasisPoints: 0 };
    ...
    const model = { schemaVersion: "police-response-model/v1", modelId: "explicit-test-police", worldId, contentHash: "",
      availableBasisPoints: 10_000, delayedBasisPoints: 0, responseMs: 1000, delayedResponseMs: 2000, identitySuccessBasisPoints: 10_000 };
    ...
    const holdPolicy: FareControlPolicyV1 = { schema: "zugfolge-fare-control-policy/v1", policyId: "explicit-test-police-hold", revision: 1,
      ...
      maxWaitMs: 60_000, policeResponseModelId: model.modelId, policeResponseModelHash: model.contentHash, publicCause: "authority.police.fare-control" };
