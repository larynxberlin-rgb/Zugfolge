...
const STATIC = [
  ...
  ...["small", "medium", "large"].flatMap((kind) => ["platform", "roof", "hall", "stairs", "underpass"].map((part) => `station.${kind}.${part}`)),
  ...["rural", "suburban", "urban"].flatMap((kind) => ["vegetation", "road", "building"].map((part) => `environment.${kind}.${part}`)),
  "signal.stop", "signal.proceed",
  ...["wheelchair", "bicycle", "stroller"].flatMap((kind) => DIRECTIONS.map((direction) => `accessory.${kind}.${direction}`)),
];
const REQUIRED = [...STATIC, ...APPEARANCES.flatMap((appearance) => DIRECTIONS.flatMap((direction) => [
...
function renderScene() {
  ...
  const trainX = 124, trainY = 394 + platformHeight - 26;
  display.dataset.trainY = String(trainY);
  draw($("roof").checked ? "vehicle.roof" : "vehicle.body", trainX, trainY, { rotated: true });
  if (!$("roof").checked) {
...
function renderGallery() {
  ...
  const all = [...new Set([...REQUIRED, ...state.assets.keys()])].sort();
...
function render() {
  ...
  const missing = REQUIRED.filter((id) => !state.assets.has(id));
  ...
  renderScene(); renderActors(); renderGallery(); renderEvidence();
}
async function load() {
...
for (const id of ["station", "environment", "roof", "platform-roof"]) $(id).addEventListener("change", renderScene);
