...
function validateToolchainSpec(value) {
  ...
  invariant(typeof value.root === "string" && isAbsolute(value.root) && /^[A-Za-z]:[\\/]/u.test(value.root), "toolchain.root muss ein expliziter absoluter Windows-Pfad sein.");
...
async function startWindowsBuildAnchor({ anchoredParents, buildParent, buildRootLeaf, hooks, spec, workspaceRoot }) {
  ...
  return {
    ...
    async extract({ sourceAudit, vendorAudit }) {
      ...
      invariant(typeof line === "string" && line.startsWith("EXTRACTED "), "Windows-Build-Anker bestaetigte die interne Slice-Extraktion nicht.");
