...
test("Alpha-Compose erzwingt Map- und Welt-Cutover-Gates, Migration, signierten Bootstrap und einen Build ohne impliziten Neubau", async () => {
  ...
  assert.match(composeWrapper, /--schema36-after-cold[\s\S]*game-schema33-migrate[\s\S]*select count\(\*\) from drizzle\.__drizzle_migrations"\)" = 36/u);
  assert.match(
    composeWrapper,
    /if \(\(keycloak_after_schema33 == 1\)\); then[\s\S]*select count\(\*\) from drizzle\.__drizzle_migrations"\)" = 36[\s\S]*keycloak-schema-backup[\s\S]*keycloak-schema-restore[\s\S]*bind-backup[\s\S]*plan-up[\s\S]*keycloak_schema_command up[\s\S]*keycloak_schema_command recover[\s\S]*keycloak_schema_command preflight-up/u,
  );
  assert.match(
    composeWrapper,
    /if \(\(prepare_v2_hot == 1\)\); then[\s\S]*select count\(\*\) from drizzle\.__drizzle_migrations"\)" = 36[\s\S]*keycloak_schema_command preflight-up[\s\S]*backup-game\.sh/u,
  );
  assert.match(
    composeWrapper,
...
test("Produktionsfreigabe dokumentiert .2 nur manuell und startet Fresh-Compose fail-closed mit .4", async () => {
  ...
  assert.match(installation, /--schema36-after-cold -f \/opt\/zugfolge\/compose\.yml/u);
  assert.match(installation, /--keycloak-after-schema36 -f \/opt\/zugfolge\/compose\.yml/u);
  assert.match(installation, /--keycloak-recover-after-schema36/u);
...
test("Produktions-Runbooks erzwingen Schema 36, Keycloak-Up und Hot-Drill in dieser Reihenfolge", async () => {
  ...
  for (const source of [installation, recovery]) {
    const schema36 = source.indexOf("--schema36-after-cold");
    const keycloakUp = source.indexOf("--keycloak-after-schema36", schema36);
    const hot = source.indexOf("--prepare-v2-hot", keycloakUp);
    assert.ok(schema36 >= 0 && keycloakUp > schema36 && hot > keycloakUp);
    assert.match(source, /preflight-up[\s\S]*Fresh-Bootstrap-Receipt[\s\S]*unzulässig/u);
  }
  assert.match(keycloak, /--keycloak-after-schema36[\s\S]*bind-backup[\s\S]*plan-up[\s\S]*`up`[\s\S]*preflight-up/u);
  assert.match(keycloak, /--keycloak-recover-after-schema36/u);
