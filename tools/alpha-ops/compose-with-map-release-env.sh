...
if (($# == 0)); then
  printf 'Aufruf: compose-with-map-release-env.sh [--prepare-v2-schema31|--prepare-v2-cold|--schema33-after-cold|--schema38-after-cold|--schema36-after-cold|--schema35-after-cold|--schema34-after-cold|--keycloak-after-schema33|--keycloak-after-schema38|--keycloak-after-schema36|--keycloak-after-schema35|--keycloak-after-schema34|--keycloak-recover-after-schema33|--keycloak-recover-after-schema38|--keycloak-recover-after-schema36|--keycloak-recover-after-schema35|--keycloak-recover-after-schema34|--prepare-v2-hot|--quiesced-cutover|--attested-rollback|--attested-rollback-stop|--fixed-stop] COMPOSE_ARGUMENTE...\n' >&2
  exit 64
fi
...
elif [[ ${1:-} == --schema33-after-cold || ${1:-} == --schema34-after-cold || ${1:-} == --schema35-after-cold || ${1:-} == --schema36-after-cold || ${1:-} == --schema38-after-cold ]]; then
  schema33_after_cold=1
  preflight_mode=pre-activation
  shift
  if (($# == 0)); then
    printf 'Der Schema-38-Gate braucht die kanonische Compose-Datei.\n' >&2
    exit 64
  fi
elif [[ ${1:-} == --keycloak-after-schema33 || ${1:-} == --keycloak-after-schema34 || ${1:-} == --keycloak-after-schema35 || ${1:-} == --keycloak-after-schema36 || ${1:-} == --keycloak-after-schema38 ]]; then
...
elif [[ ${1:-} == --keycloak-recover-after-schema33 || ${1:-} == --keycloak-recover-after-schema34 || ${1:-} == --keycloak-recover-after-schema35 || ${1:-} == --keycloak-recover-after-schema36 || ${1:-} == --keycloak-recover-after-schema38 ]]; then
...
if [[ "$compose_action" == run ]] && ((prepare_v2_schema31 == 0 && prepare_v2_cold == 0 && schema33_after_cold == 0 && keycloak_after_schema33 == 0 && keycloak_recover_after_schema33 == 0)); then
  ...
  for argument in "$@"; do
    if [[ "$argument" == game-migrate || "$argument" == game-schema31-migrate || "$argument" == game-schema31-qualify || "$argument" == game-schema33-migrate ]]; then
      printf 'Eine direkte Schema-Migration ist gesperrt; initial sind ausschliesslich --prepare-v2-schema31 und --schema38-after-cold zulaessig.\n' >&2
      exit 64
    fi
...
if ((quiesced_cutover == 1 || attested_rollback == 1 || attested_rollback_stop == 1 || prepare_v2_schema31 == 1 || prepare_v2_cold == 1 || schema33_after_cold == 1 || keycloak_after_schema33 == 1 || keycloak_recover_after_schema33 == 1 || prepare_v2_hot == 1)); then
  ...
  if ((schema33_after_cold == 1)); then
    ...
    run_recovery_material -eu -c '
      test "$(psql "$PRODUCTION_COLD_GAME_DATABASE_URL" -X -v ON_ERROR_STOP=1 -Atc "select count(*) from drizzle.__drizzle_migrations")" = 38
    '
    preparation_completed=1
    trap - EXIT INT TERM
    cleanup_compose_env_snapshot
