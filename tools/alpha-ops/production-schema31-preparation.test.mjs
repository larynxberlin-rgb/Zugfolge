...
test("Schema-31 preparation accepts the current vehicle-register image and retains historical journal contracts", async () => {
  ...
  for (const length of [33, 34, 35, 36]) {
  ...
  missingRegistry.entries[35].tag = "0036_unapproved_change";
  assert.throws(() => validateSchema31ImageJournal(missingRegistry), /Migration 0036/u);
  assert.throws(() => validateSchema31ImageJournal({ ...journal, entries: [...journal.entries, { tag: "0037_future" }] }), /Journalvertrag/u);
});
