...
if (["operations", "program", "reports"].includes(screen)) {
  ...
  if (screen === "planner") {
    ...
    const render = (accepted: boolean) => {
      ...
      root.innerHTML = renderJourney({publicWorldId:worldId,busy:false,message:"",activeSection:founding?"world":screen==="workshop"?"operations":screen as never,entryConfirmed:accepted,hasActiveOperator:!founding,activeOperatorId:founding?"":operatorId,livemapUrl:"http://127.0.0.1:4173/?screen=map",operationsCenterUrl:"http://127.0.0.1:4173/?screen=operations",worldContracts:[worldContract] as never,operatorContext:founding?{...playerContext,operators:[]}:playerContext as never,mailbox:mailbox as never,cooperation:{...marketState,section:screen==="markets"?"markets":screen==="workshop"?"operations":"all",activeOperatorId:founding?"":operatorId} as never});
