...
Nur dieser Modus darf danach die initiale DDL von 0032 bis 0036 bis Zielkopf 36 starten:
...
Wrapper verlangt exakt 36 Migrationen einschließlich
`0036_persistent_vehicle_registry`. Der mit Migration 0033 eingeführte weltgebundener Ledger trägt
...
Dieser einzige Produktionsmodus prüft erneut den exakten Schema-36-Kopf,
...
`--keycloak-recover-after-schema36` wieder aufgenommen. Jeder andere Fehler
...
Vor dem ersten Dump prüft `--prepare-v2-hot` Schema 36 und den installierten
Keycloak-Up-Vertrag erneut. Er erzeugt dann neue Schema-36-Dumps und
...
Schema-36-Hot-Restore deshalb ausschließlich mit `KC_DB_SCHEMA=keycloak`.
