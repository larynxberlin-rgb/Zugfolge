...
aufgerufen. Nach dem erfolgreichen `--schema36-after-cold` führt ausschließlich
...
Prüfdatenbankcontainer und verlangt zunächst exakt 36 Drizzle-Migrationen. Dann
...
vorhandenen create-new Artefakte wieder auf. Er verlangt Schema 36, einen
...
```bash
bash tools/alpha-ops/compose-with-map-release-env.sh \
  --keycloak-recover-after-schema36 -f /opt/zugfolge/compose.yml
```
...
Schema-36-Hot-Restore und muss deshalb weiterhin `KC_DB_SCHEMA=keycloak`
