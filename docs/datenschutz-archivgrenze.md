...
## Gemeinsame Integration mit Schema 38

Migration 0038 ergänzt das persistente Fahrzeugregister nach den unveränderten
Migrationen 0036 für Schaffnersitzungen und 0037 für Archivredaktion.
Der gemeinsame Tabellen-/Triggervertrag qualifiziert Schema 38 als
`zugfolge-database-rollback-proof/v8`. Neue Weltsiegel verwenden
`zugfolge-world-final-history-seal/v5` und binden zusätzlich die beiden
Registertabellen sowie die neuen Fahrzeugzustands- und Bewertungsfelder.
Historische v1- bis v4-Siegel behalten ihre ursprüngliche Bedeutung; sie
dürfen neue Fahrzeugfakten nicht ausblenden.

Die Redaktionsbrücke gilt auch nach diesem Upgrade. Fahrzeugregister und
Lebenslauf gehören nicht zu ihren erlaubten persönlichen Tabellen und bleiben
bei Kontopurge, Postfachpurge und Restore unverändert. Die Integrationstests
verwenden Archive aus Schema 33, 36, 37 und 38, ursprüngliche Backupbytes und
unabhängig gepinnte Redaktionsaufträge. Schema 38 enthält dabei bereits einen
Fahrzeugpass samt Historie. Die früheren Schema-36-/37-Nachweise im folgenden
Implementierungsvertrag werden dadurch nicht rückwirkend erweitert.
