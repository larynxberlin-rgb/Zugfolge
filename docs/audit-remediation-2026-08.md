...
| #48 | Der Vergleichsfehler ist behoben: S5/S5X werden getrennt, GTFS-Sollfahrzeit, Zwischenhalt und unabhängige technische Referenz sind unterschiedliche Größen. Eine manuelle Trassenfinder-Einzelabfrage liefert 1.260 Sekunden; die deterministische Abschnittskalibrierung rechnet 1.263 Sekunden (+3 s bei ±63 s). Der GTFS-Holdout aus 85 S5X-Fahrten weist 1.380 Sekunden P20 aus, davon 60 Sekunden Halt und 57 Sekunden verbleibende Fahrplanreserve. Der Report besteht, bleibt aber korrekt `calibration-only` und nicht signierbar. Offen sind eine unabhängige Validierung auf eingefrorenen, belastbaren Infrastruktur-/Fahrzeugprofilen, die benannte Release-Identität und deren echte Signatur. |
