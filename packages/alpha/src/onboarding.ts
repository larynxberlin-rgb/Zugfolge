import { accounts, alphaWorldProfiles, domainEvents, onboardingGrants, worlds } from "@zugfolge/db";
...
function validateSpec(spec: StartPackageSpec): void {
  if (spec.schemaVersion !== "zugfolge-start-package/v1" || spec.version.trim() === "") throw new AlphaValidationError("Startpaket-Spezifikation ist ungueltig.");
...
export class OnboardingService {
  ...
  async claim(worldId: string, keycloakSubject: string, atS: number, spec: StartPackageSpec) {
    ...
    if (profile?.profileKind !== "public" || profile.state !== "running") throw new AlphaConflictError("Startpaket ist nur in einer laufenden oeffentlichen Welt verfuegbar.");
    ...
    return this.db.transaction(async (tx) => {
      ...
      if (proof.lotId !== spec.emergencyLotId || proof.personnelPoolId !== spec.personnelPoolId || !proof.operatingProgramActive) throw new AlphaConflictError("Autoritativer Startpaketbeleg ist unvollstaendig.", "start_package_incomplete");
  ...
  async capacityHeatmap(worldId: string, fromS: number, untilS: number) {
    if (!Number.isSafeInteger(fromS) || !Number.isSafeInteger(untilS) || fromS < 0 || untilS <= fromS || untilS - fromS > 7 * 86_400) throw new AlphaValidationError("Heatmap-Zeitraum ist ungueltig.");
    const cells = await this.port.capacityCells(worldId, fromS, untilS);
    return cells.map((cell) => {
      if (cell.intervalEndS <= cell.intervalStartS || cell.usedSeconds < 0 || cell.capacitySeconds <= 0 || cell.usedSeconds > cell.capacitySeconds) throw new AlphaConflictError("Kapazitaetsquelle lieferte ungueltige Ganzzahlwerte.");
      ...
      return {
        ...
        stateLabel: cell.qualityClass === "C" ? "sichtbar, nicht bestellbar" : utilizationBasisPoints >= 9_000 ? "nahezu belegt" : utilizationBasisPoints >= 7_000 ? "angespannt" : "verfuegbar",
  ...
  operatingManagerWarnings(input: { readonly pathConfirmed: boolean; readonly vehicleAvailable: boolean; readonly personnelCovered: boolean; readonly operatingProgramActive: boolean; readonly maintenanceDueInS: number | null; readonly capacityBasisPoints: number }): readonly { readonly code: string; readonly severity: "info" | "warning" | "blocking"; readonly message: string }[] {
    ...
    if (!input.pathConfirmed) warnings.push({ code: "path-missing", severity: "blocking", message: "Keine bestaetigte Trasse: der Betrieb kann nicht starten." });
    if (!input.vehicleAvailable) warnings.push({ code: "vehicle-missing", severity: "blocking", message: "Kein verfuegbares und zugelassenes Fahrzeug." });
    if (!input.personnelCovered) warnings.push({ code: "personnel-missing", severity: "blocking", message: "Personalbedarf ist nicht vollstaendig gedeckt." });
    ...
    if (input.capacityBasisPoints >= 9_000) warnings.push({ code: "capacity-tight", severity: "warning", message: "Gewaehltes Kapazitaetsfenster ist nahezu belegt." });
    return warnings;
  }
}
