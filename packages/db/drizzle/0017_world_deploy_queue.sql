...
ALTER TABLE "odoo_command_queue" DROP CONSTRAINT "odoo_command_queue_world_fk";--> statement-breakpoint
ALTER TABLE "odoo_projection_outbox" DROP CONSTRAINT "odoo_projection_outbox_world_fk";--> statement-breakpoint
ALTER TABLE "alpha_world_profiles" ADD COLUMN "deployment_hash" text;--> statement-breakpoint
CREATE TABLE "alpha_world_deployments" (
	...
	CONSTRAINT "alpha_world_deployments_deployment_hash_unique" UNIQUE("deployment_hash"),
	CONSTRAINT "alpha_world_deployments_hash_format" CHECK ("deployment_hash" ~ '^[0-9a-f]{64}$')
...
--> statement-breakpoint
CREATE TABLE "global_admin_audit_events" (
	...
	CONSTRAINT "global_admin_audit_events_outcome_check" CHECK ("outcome" = 'rejected')
);--> statement-breakpoint
...
CREATE UNIQUE INDEX "global_admin_audit_events_command_idx" ON "global_admin_audit_events" USING btree ("command_id");--> statement-breakpoint
-- guards:allow world-id — target_world_id ist die noch nicht erzeugte Zielwelt und daher bewusst kein world_id-FK.
CREATE INDEX "global_admin_audit_events_target_world_idx" ON "global_admin_audit_events" USING btree ("target_world_id","occurred_at");--> statement-breakpoint
CREATE FUNCTION "zugfolge_enforce_global_admin_audit"() RETURNS trigger AS $$
BEGIN
	IF NOT EXISTS (
...
CREATE FUNCTION "zugfolge_enforce_odoo_command_world"() RETURNS trigger AS $$
BEGIN
...
CREATE FUNCTION "zugfolge_enforce_odoo_outbox_world"() RETURNS trigger AS $$
BEGIN
...
--> statement-breakpoint
CREATE FUNCTION "zugfolge_protect_started_alpha_world_profile"() RETURNS trigger AS $$
BEGIN
