...
export const COST_TYPES = ECONOMY_COST_TYPES;
...
export function calculateProfitAndLoss(revenueCents: bigint, postings: readonly ClassifiedPosting[]): ProfitAndLoss {
  ...
  const costs = Object.fromEntries(ECONOMY_COST_TYPES.map((type) => [type, postings
  ...
  return Object.freeze({ revenueCents, costsByType: Object.freeze(costs), resultCents: result, explanation: Object.freeze([`Erlöse ${revenueCents}`, ...ECONOMY_COST_TYPES.map((type) => `${type} ${costs[type]}`), `Ergebnis ${result}`]) });
}
