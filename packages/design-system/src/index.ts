...
export type IconName = "alert" | "train" | "layers" | "chevron" | "check" | "close" | "clock" | "route" | "lock" | "warning";
...
const paths: Readonly<Record<IconName, string>> = {
  ...
  chevron: '<path d="m9 5 7 7-7 7"/>', check: '<path d="m4 12 5 5L20 6"/>', close: '<path d="m5 5 14 14M19 5 5 19"/>',
  clock: '<circle cx="12" cy="12" r="9"/><path d="M12 7v6l4 2"/>', route: '<circle cx="6" cy="18" r="2"/><circle cx="18" cy="6" r="2"/><path d="M8 18h3a3 3 0 0 0 3-3V9a3 3 0 0 1 3-3"/>',
  lock: '<rect x="5" y="10" width="14" height="10" rx="2"/><path d="M8 10V7a4 4 0 0 1 8 0v3"/>', warning: '<path d="M12 4v10m0 4v.2"/><circle cx="12" cy="12" r="9"/>',
};
...
export function field(id: string, label: string, value: string, help?: string): string {
...
export function emptyState(title: string, description: string): string { return `<div class="zf-empty" role="status">${icon("route")}<strong>${escapeHtml(title)}</strong><span>${escapeHtml(description)}</span></div>`; }
