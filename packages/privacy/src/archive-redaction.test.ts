...
it.each([33, 36, 37, 38])("redigiert ein echtes Schema-%i-Archiv und restauriert alte Backupbytes ausschließlich mit unabhängig gepinntem Löschbeleg", async (schemaCount) => {
  ...
  const historySchema = `zugfolge-world-final-history-seal/v${schemaCount === 33 ? 1 : schemaCount === 38 ? 5 : 4}`;
  try {
    ...
    if (schemaCount === 38) {
      await client.query(`insert into vehicle_registry_entries(world_id,vehicle_id,authority_release_id,class_designation,
        owner_operator_id,holder_operator_id,introduced_at_s,retired_at_s,data_at_s,fleet_revision,
        source_state_hash,facts,facts_hash,history_hash)
        values($1,'archive-vehicle','test-release','Testfahrzeug','public-operator','public-operator',0,10000,1,1,$2,'{"syntheticVehicle":"retained"}',$2,$2)`, [WORLD, "c".repeat(64)]);
      await client.query(`insert into vehicle_registry_events(world_id,vehicle_id,fleet_revision,at_s,event_type,
        resulting_history_hash,source_state_hash,details)
        values($1,'archive-vehicle',1,1,'registered',$2,$2,'{"syntheticVehicle":"retained"}')`, [WORLD, "c".repeat(64)]);
    }
    ...
    const registryBefore = {
      entries: (await client.query("select * from vehicle_registry_entries where world_id=$1", [WORLD])).rows,
      events: (await client.query("select * from vehicle_registry_events where world_id=$1", [WORLD])).rows,
    };
    expect(registryBefore.entries).toHaveLength(schemaCount === 38 ? 1 : 0);
    ...
    expect((await client.query("select * from vehicle_registry_entries where world_id=$1", [WORLD])).rows).toEqual(registryBefore.entries);
    expect((await client.query("select * from vehicle_registry_events where world_id=$1", [WORLD])).rows).toEqual(registryBefore.events);
    if (schemaCount === 38) {
      await expect(client.query("delete from vehicle_registry_entries where world_id=$1", [WORLD])).rejects.toThrow();
      await expect(client.query("update vehicle_registry_events set details='{}' where world_id=$1", [WORLD])).rejects.toThrow();
    }
    ...
    expect(cleanSource.migrationLedger).toHaveLength(38);
